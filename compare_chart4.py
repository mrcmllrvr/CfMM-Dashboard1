...
def register_callbacks(app):
    ...
    #     filtered_df_4b = df.copy()

    #     if selected_start_date and selected_end_date:
    #         start_date = pd.to_datetime(selected_start_date)
    #         end_date = pd.to_datetime(selected_end_date)
    #         filtered_df_4b = filtered_df_4b[(filtered_df_4b['publish_date'] >= start_date) & (filtered_df_4b['publish_date'] <= end_date)]
    #     if selected_publishers:
    ...
    def update_table4a(n_clicks_search, n_clicks_clear, selected_start_date, selected_end_date, selected_publishers, selected_topics, selected_bias_categories, selected_bias_ratings, selected_ngrams, text_by, search_word):
        ...
        if triggered:
            ...
            if id == 'search-button4a':
                ...
                # Dash
                filtered_df = filtered_df.sort_values('publish_date_label_(yyyy-mm-dd)', ascending=False)[['publisher', 'title_label', 'publish_date_label_(yyyy-mm-dd)', 'topic_name', 'bias_rating_label', 'generalization_tag', 'omit_due_prominence_tag', 'negative_aspects_tag', 'misrepresentation_tag', 'headline_bias_tag', 'explore_further']]
