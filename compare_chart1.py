...
def create_layout():
    layout = html.Div(style={'justify-content': 'center', 'backgroundColor': '#ffffff'}, className='row', children=[
        html.H3(children=f"""Who are the top offending publishers during the selected period?""", style={'textAlign': 'center', 'font-weight': 'bold', 'margin-bottom': '30px'}),
        ...
        if triggered:
            ...
            if id in ['top-offending-publishers-bar-chart-1a', 'export-button1a']:
                ...
                if selected_bias_categories:
                    ...
                    filtered_df = filtered_df[filtered_df[selected_bias_categories].sum(axis=1) > 0]
                if selected_topics:
                    filtered_df['topic'] = filtered_df['topic'].fillna("")
                ...
                if (clickData is not None) or (clickData is None & id == 'export-button1'):
                    ...
                    if color_by == 'bias_ratings':
                        ...
                        # Save to csv
                        csv_df = filtered_df[['publisher', 'headline', 'url', 'publish_date_label_(yyyy-mm-dd)', 'topic', 'bias_rating_label'] + categories]
                        ...
                        # Dash
                        filtered_df = filtered_df.sort_values('publish_date_label_(yyyy-mm-dd)', ascending=False)[['publisher', 'title_label', 'publish_date_label_(yyyy-mm-dd)', 'topic', 'bias_rating_label'] + categories + ['explore_further']]
